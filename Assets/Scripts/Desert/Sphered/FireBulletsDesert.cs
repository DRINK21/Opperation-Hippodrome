...
public class FireBulletsDesert : MonoBehaviour
{
    // Phase 1 
    ...
    void Update()
    {
        ...
        if (SphereMovement.bossHealth >= 71)  // Stage 1
        {
        ...
        if ((SphereMovement.bossHealth >= 31 && SphereMovement.bossHealth < 71)) // Stage 2
        {
    ...
    private void Fire()
    {
        ...
        for (int i = 0; i < bulletsAmount + 1; i++)
        {
            ...
            GameObject bul = BulletPoolD.bulletPoolInstanse.GetBullet();
            bul.transform.position = transform.position;
            bul.transform.rotation = transform.rotation;
            bul.SetActive(true);
            bul.GetComponent<SphereBullet>().SetMoveDirection(bulletDir);
